...
class RegisterAsDistributor extends StatefulWidget {
  @override
  _RegisterAsDistributorState createState() => _RegisterAsDistributorState();
}

class _RegisterAsDistributorState extends State<RegisterAsDistributor> {
            ...
            child: Scaffold(
              ...
              body: SingleChildScrollView(
                child: Column(
                  children: <Widget>[
                    AuthHeader(
                      headerTitle: Strings.RegisterAsADistributor,
                    ...
                    Form(
                      ...
                      child: Container(
                        padding:
                            EdgeInsets.only(left: 40.0, right: 40.0, top: 20),
                        child: Column(
                          children: [
                            SizedBox(height: 10),
                            TextFormField(
                              ...
                              onTap: () async {
                                try {
                                  File file = await FilePicker.getFile(
                                      type: FileType.image);
                                  uploadDocumentWithParamToServer(
                                      documentType: 'gst_document', file: file);
  ...
  Future getStatesWithReferenceTo(
      {int pageNumber, String countryID = '101', BuildContext context}) async {
    if (GlobalModelClass.internetConnectionAvaiable() == true) {
    ...
    if (GlobalModelClass.internetConnectionAvaiable() == true) {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonDecode(response.body)['status'].toString() == 'true') {
          ...
          ToastUtils.showError(
            message: responseDict['response']['message']?.toString() ??
                Strings.PleaseCheckForAllMandatoryFields,
          );
        }
      } else if (response.statusCode == 500) {
        ToastUtils.showError(message: Strings.internalServerError);
      } else {
